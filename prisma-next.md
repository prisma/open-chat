...
- **PostgreSQL 17 or newer.** Older servers are not supported. Run `SELECT version()` against your server to verify.
...
```bash
bun prisma-next contract emit       # Update contract.json and contract.d.ts
bun prisma-next db init             # Create tables in the database
bun prisma-next migration status    # Show migration status
```
